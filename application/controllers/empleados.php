...
class Empleados extends CI_Controller {
    ...
    function mostrarAnuncio() {
        ...
        if ($this->input->is_ajax_request()) {
            $buscar = $this->input->post("buscar");
            $datos = $this->Empleados_model->mostrarAnuncios($buscar,$id);
            echo json_encode($datos);
        } else {
            show_404();
        }
    ...
    function eliminar2() {
        if ($this->input->is_ajax_request()) {
            $idsele = $this->input->post("id");
            $registro = $this->Empleados_model->capturarImagen2($idsele);
            unlink("./img/cargas/" . $registro->archivo);
            if ($this->Empleados_model->eliminar2($idsele) == true)
                echo "Registro Eliminado";
            else
                echo "No se pudo eliminar los datos";
        }
...
//			$dni = $this->input->post("dnisele");
            ...
            if ($this->upload->do_upload('foto_nueva')) {
                ...
                if ($this->Empleados_model->actualizar2($idsele, $datos,$mail) == true) {
                    echo "Registro Actualizado";
                } else {
                    echo "No se pudo actualizar los datos";
                }
                ...
                if ($this->Empleados_model->actualizar2($idsele, $datos) == true) {
                    echo "Registro Actualizado";
                } else {
                    echo "No se pudo actualizar los datos";
                }
